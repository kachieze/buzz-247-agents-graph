...
resource "aws_security_group" "agent" {
  ...
  description = "Fargate agents: egress all, inbound none except NFS via EFS SG"
  vpc_id      = aws_vpc.this.id
  egress {
    from_port   = 0
    to_port     = 0
...
resource "aws_security_group" "relay" {
  ...
  description = "In-stack Buzz relay tasks"
  vpc_id      = aws_vpc.this.id
  egress {
    from_port   = 0
    to_port     = 0
  ...
  tags = merge(var.tags, { Name = "${var.name_prefix}-relay-sg" })
}

resource "aws_security_group" "alb" {
  name        = "${var.name_prefix}-alb"
  description = "Public ALB for optional relay"
  vpc_id      = aws_vpc.this.id
  ingress {
    from_port   = 443
    to_port     = 443
...
resource "aws_security_group_rule" "relay_from_alb" {
  type                     = "ingress"
  from_port                = 3000
  to_port                  = 3000
  protocol                 = "tcp"
