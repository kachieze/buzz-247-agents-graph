...
resource "aws_secretsmanager_secret" "rds_password" {
  count                   = var.create_secret_shells && var.relay_enabled ? 1 : 0
  name                    = "${var.name_prefix}/relay/rds-password"
  recovery_window_in_days = 0
  tags                    = var.tags
}
...
locals {
  ...
  rds_password_arn      = try(aws_secretsmanager_secret.rds_password[0].arn, try(data.aws_secretsmanager_secret.rds_password[0].arn, ""))
...
output "rds_password_arn" { value = local.rds_password_arn }
